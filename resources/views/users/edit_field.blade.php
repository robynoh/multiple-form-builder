...
<br/>
               ...
               <a href="/users/create_form_edit/{{UserController::encrypt_decrypt($table,true)}}"  class="button w-32 mr-2 mb-2 flex items-center justify-center bg-theme-7 text-white"> <i data-feather="chevron-left" class="w-4 h-4 mr-2"></i> Go Back </a>             
...
{{ method_field("PUT")}}
 {{ csrf_field() }} 
                       ...
                       <div> <label><b>Question</b></label>
                        ...
                        <input name="table" id="table"  type="hidden" class="input w-full border mt-2" placeholder="Enter your question here" value="{{ $table }}"> </div>
...
$(document).ready(function(){
  ...
  $.ajax({
    url:'/users/displayprocessoption',
      method:'POST',
      data: {
        title:title,tableid:tableid,option_type:option_type,optionValue:optionValue,_token:_token
      },
   ...
   var tableid=$('#table').val();
     ...
     $.ajax({
         url:'/users/deleteoption',
           method:'POST',
           data: {
             title:title,tableid:tableid,option_type:option_type,optionid:optionid,_token:_token
           },
