...
@section('content')
                ...
                <div class="top-bar">
                    <!-- BEGIN: Breadcrumb -->
                    <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="" class="">Admin</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Articles</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
...
<button name="action" value="delete"  class="button w-45 mr-2 mb-2 flex items-center justify-center bg-theme-6 text-white"> <i data-feather="trash-2" class="w-4 h-4 mr-2"></i>Delete All</button>  
                            ...
                            <a href="javascript:;" data-toggle="modal" data-target="#superlarge-modal-size-preview"   class="button w-45 mr-2 mb-2 flex items-center justify-center bg-theme-7 text-white"> <i data-feather="plus" class="w-4 h-4 mr-2"></i>Add Article </a>  
                           ...
                           <a href="/admin/categories"   class="button w-45 mr-2 mb-2 flex items-center justify-center bg-theme-11 text-white"> <i data-feather="list" class="w-4 h-4 mr-2"></i>Category </a>  
                     ...
                     <b class="intro-y  mt-10">
                   Status
</b>
                     <table><tr><td>
                        <select name="activity" class="input border mr-2">
             
             <option value="1">Publish</option>
             <option value="0">Unpublish</option>
         </select> </td><td><button name="action" value="status" class="button w-45 mr-2 flex items-center justify-center bg-theme-3 text-white"> Action </button>  
</td></tr></table>
...
<div class="grid grid-cols-12 gap-6 mt-1">
                    ...
                    <table class="table table-report table-report--bordered display datatable w-full" style="font-size:14px;">
                       ...
                       @foreach($blogs as $blog)
                            <tr >
                            <td class="border-b"> <input name="blog[]" type="checkbox" class="input border mr-2" id="vertical-checkbox-daniel-craig" value="{{$blog->id}}">
                        </td>

                        <td class="border-b" >
                       <span style="font-weight:bold;font-size:large"> {{$blog->title}} </span><br/>
                       <span style="">{!! ucfirst(substr($blog->content,0,100)) !!} </span>
                                    </td>
                            ...
                            <b>{{$blog->category}}</b>     
                              ...
                              <td class="border-b">
                              <?php if($blog->status==1){?> <span class="text-xs px-1 rounded-full bg-theme-9 border text-white mr-1"> Published</span> <?php }else{?> <span class="text-xs px-1 rounded-full bg-theme-6 text-white mr-1">Unpublished</span> <?php }?>
                                </td> 
                                
                              
                                <td class="w-40 border-b">
                                <div class="font-medium whitespace-no-wrap ">  <a href="/admin/blog/{{$blog->id}}"  ><i data-feather="edit"></i></a></div>
                                </td>

                            <td class="w-40 border-b">
                                <div class="font-medium whitespace-no-wrap text-theme-6">  <a  id="deleteUser{{ $blog->id}}" data-userid="{{$blog->id}}" href="javascript:void(0)" onclick="showAlertfrm3({{ $blog->id}});"  ><i data-feather="trash-2"></i></a></div>
                                </td>
...
<div class="relative w-full mx-auto mt-5" style="text-align:left">
     ...
     <input required name="title" type="text" class="input pl-12 w-full border col-span-4" placeholder="Title" style="font-size:30px" required>
 </div>


 <div class="relative w-full mx-auto mt-5" style="text-align:left">
       <label style="">Seo Link Phrase</label>    </div>

       <div class="relative">
<div class="absolute rounded-l w-10 h-full flex items-center justify-center bg-gray-100 border text-gray-600"><i data-feather="list" class="w-4 h-4"></i></div> 
<input required  name="seo" type="text" class="input pl-12 w-full border col-span-4" placeholder="Seo Link Phrase"  required>
</div>
...
<div class="relative w-full mx-auto mt-5" style="text-align:left">
       <label style="font-size:40px">Seo Link Phrase</label>    </div>

       <div class="relative">
<div class="absolute rounded-l w-10 h-full flex items-center justify-center bg-gray-100 border text-gray-600"><i data-feather="list" class="w-4 h-4"></i></div> 
<input required  name="link-phrase" type="text" class="input pl-12 w-full border col-span-4" placeholder="Seo Link Phrase" style="font-size:30px" required>
</div>
               ...
               {{ csrf_field() }}
                    ...
                    <div class="modal__content">
                        <div class="p-5 text-center">
                            ...
                            <div class="text-gray-600 mt-2" style="font-size:15px"><b id="niche"></b> <br/>Are you sure you want to delete this blog post?</div>
...
window.location="/admin/blogpost/delete/"+$('#app_id3').val();
 

}

function selectAll(source) {
    checkboxes = document.getElementsByName('blog[]');
    for(var i in checkboxes)
        checkboxes[i].checked = source.checked;
}
